"""월요일에 그 주 이미지(요일당 1~3장)+캡션 초안을 pending_posts.json과 옵시디언 노트로 저장한다.
이미지 생성 자체(힉스필드 호출)는 Claude가 매주 월요일에 직접 수행하고,
그 결과(이미지 파일들 + 캡션 텍스트)를 이 스크립트의 save_week()에 넘겨 저장한다.
"""
import os
import json
import shutil
...
def save_week(day_plans: dict, monday: date = None):
    """day_plans: {"Monday": {
        "images": [{"image_path": "<repo 내 images/... 절대/상대경로>", "image_url": "<힉스필드 rawUrl>"}, ...],  # 1~3장
    ...
    for i, (day_en, day_kr) in enumerate(zip(DAYS_EN, DAYS_KR)):
        ...
        for img in info.get("images", []):
            ...
            image_entries.append({
                "path": f"images/{week_dir_name}/{image_filename}",
                "image_url": img.get("image_url", ""),
            })
            embed_lines.append(f"![[{image_filename}]]")
        ...
        lines.append(f"## {day_kr} ({post_date.month}/{post_date.day}) — {info.get('product','')} / {info.get('hook','')} ({len(image_entries)}장)\n")
        lines.append("\n".join(embed_lines) + "\n")
        lines.append("캡션:")
