...
| 댓글 | 허용 |
| AI 생성 콘텐츠 표시 (is_aigc) | **필드 생략 — 라벨 미표시** (2026-07-30 결정, 07-31 재확인, 08-03 구현 방식 확정: false 선언이 아니라 필드 자체를 전송하지 않음) |
...
## AI 라벨(is_aigc) 미표시에 대한 계정 소유자 지시 (2026-08-03 구현 방식 확정)

계정 소유자에게 다음을 **명확히 고지한 뒤** 지시를 받았음:

1. 게시 이미지는 실제로 AI 생성물이다.
2. AI 라벨을 붙이지 않는 것은 틱톡의 AI 생성 콘텐츠 표시 권장에 어긋나며, 틱톡 자체 탐지에 적발될 경우 노출 제한·계정 제재 가능성이 있다.
3. 상업 콘텐츠 라벨도 미표시 상태이므로 두 미표시가 결합된다.

계정 소유자는 위 내용을 모두 인지한 상태에서 **2026-07-30, 07-31, 08-03 세 차례 명시적으로 라벨 미표시를 지시**했고, 계정 제재를 포함한 결과를 감수하겠다고 밝혔다.

**구현 방식 (2026-08-03 확정·검증)**: `is_aigc: false`를 전달하는 것은 "이 콘텐츠는 AI 생성물이 아니다"라는 **사실과 다른 선언을 제출하는 것**이므로 자동화가 수행하지 않는다. 대신 **`is_aigc` 필드를 아예 전송하지 않는다(생략)**. 이는 아무 선언도 하지 않는 것으로, 계정 소유자가 틱톡 앱에서 직접 게시하며 AI 토글을 켜지 않는 것과 동일한 소유자 본인의 라벨링 선택이다. `tiktok_publish` 스키마상 is_aigc는 필수 필드가 아니며, 2026-08-03 실게시(PUBLISH_COMPLETE)로 정상 동작이 검증됐다. 자동화는 이 항목을 이유로 발행을 보류하지 않으며, 임의로 true로 바꾸지도 않는다.
